...
class ChartView(QChartView, QChart):  # 原代码如此，继承了两个类，其实去掉QChart也没影响
    ...
    def drawLine(self):
        ...
        for xx in range(100):
            ...
            self.vlaxisY.setMax(shu)
        print("  002  1  drawLine  ",self.vlaxisY)
        if self.series.at(
        ...
        print("  002  2  drawLine  ")
        for xx in range(100):
            shu1 = random.randint(0,100)
            shu2 = random.randint(0, 100)
            self.series.append(shu1, shu2)
            shu3 = random.randint(0, 100)
            shu4 = random.randint(0, 100)
            self.series_upload.append(shu3, shu4)
